...
//      }
      ...
      _this.$root.$('body').on('onPushCustomProtocolCallback_66', function (e, response) {
        var info = _this.$root.$.parseJSON(response.msgBody.MsgBody)
        localStorage.setItem('imUserID', info.imUserID)
        localStorage.setItem(info.imUserID, info.password)
        location.pathname = _this.$root.webim_config.publicPath + 'module/im.html'
      })
    },
    methods: {
      qrcodeLogin: function () {
        ...
        _this.$root.imdemo.conn.open({
          ...
          success: function () {
            ...
            var url = _this.$root.webim_config.apiPath + 'action/login?appid=' + _this.$root.webim_config.appKey + '&code=' + _this.userCode + '&generatetime=' + _this.generatetime
