    ...
    <div v-for="(msgBoxItem,key) in cache.msgBox">
      <li v-if="msgBoxItem.type === 'addFriend'">
        ...
        <p class="layim-msgbox-btn">
          <button class="layui-btn layui-btn-small" @click="agreeFriend(msgBoxItem, key)">同意</button>
          <button class="layui-btn layui-btn-small layui-btn-primary" @click="refuseFriend(msgBoxItem, key)">拒绝</button>
        </p>
      </li>
    </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      agreeFriend: function (msgBoxItem, position) {
        var _this = this
        _this.$root.$.ajax({
          type: 'post',
          ...
          success: function (data) {
            if (data.code === 1) {
              ...
              _this.$root.eventHub.$emit('addFriend', {
                groupId: _this.$root.imdemo.conn.generateRouteid(_this.cache.mine.id, msgBoxItem.friendImID)
              })
            } else {
              _this.$root.layer.msg(data.code + ':' + data.msg)
            }
            _this.cache.msgBox.splice(position, 1)
          }
        })
      },
      refuseFriend: function (msgBoxItem, position) {
        var _this = this
        _this.$root.$.ajax({
          type: 'post',
          url: _this.$root.webim_config.apiPath + 'friend/auditFriend',
