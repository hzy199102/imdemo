...
<script>
  import mainPanel from './basic/main/mainPanel.vue'
  import minPanel from './basic/main/minPanel.vue'
  import mainChatPanel from './basic/chat/mainChatPanel.vue'
  import mainChatMinPanel from './basic/chat/mainChatMinPanel.vue'
  import chatLogPanel from './basic/log/chatLogPanel.vue'
  import Vue from 'vue'
  export default {
    data () {
      return {
...
//        console.log(response)
      ...
      _this.eventHub.$on('addFriend', function (msg) {
        _this.friendList()
        _this.cache.msgAll[msg.groupId].allowTalk = true
      })
      _this.eventHub.$on('removeFriend', function (msg) {
        _this.friendList()
        _this.cache.msgAll[msg.groupId].allowTalk = false
      })
//      _this.eventHub.$on('addGroup', function (msg) {
//        _this.listGroup()
//      })
//      _this.eventHub.$on('removeGroup', function (msg) {
//        _this.listGroup()
//      })
      ...
      _this.eventHub.$on('mineMessage', function (msg) {
        ...
        if (!_this.cache.msgAll[groupId].allowTalk) {
          console.log('已经删除好友关系或者已经退群的聊天')
          msg.status = 2
          _this.cache.msgAll[groupId].msgArray.push(msg)
          var content = ''
          if (_this.cache.msgAll[groupId].groupType === 1) {
            content = '你们已经不是好友关系'
          } else if (_this.cache.msgAll[groupId].groupType === 2) {
            content = '你已经不是群成员'
          }
          _this.cache.msgAll[groupId].msgArray.push({
            type: 'sys',
            content: content
          })
          return false
        ...
        var position = _this.cache.msgAll[groupId].msgArray.length - 1
        var options = {
          msgId: msgId,
          touid: _this.cache.msgAll[groupId].groupType === 1 ? _this.cache.msgAll[groupId].basicInfo.friendImID : groupId,
          groupName: _this.cache.msgAll[groupId].basicInfo.userName,
          groupId: groupId,
    ...
    methods: {
        ...
        if (_this.cache.msgAll[msg.groupId].seq > _this.cache.msgAll[msg.groupId].readSeqid) {
          ...
          var options = {
            ...
            success: function (response) {
              if (_this.cache.msgAll[msg.groupId].readSeqid <= parseInt(response.msgBody.seqid)) {
                _this.cache.msgAll[msg.groupId].readSeqid = parseInt(response.msgBody.seqid)
                _this.cache.msgAll[msg.groupId].unread = 0
              }
            },
            fail: function (response) {
              _this.$root.imdemo.utils.onError({
                code: 'onPostReadSeqFailedCallback',
                message: '上报最大阅读数失败！',
                data: response
              })
            }
      ...
      pullMsg: function () {
        ...
        if (!_this.cache.msgAll[groupId].pullSession) {
          ...
          _this.$root.imdemo.conn.pullSession({
            sessionCode: groupId,
            success: function (response) {
              _this.pullSession2Success(groupId, response)
              if (response.msgBody.SessionList.length === 0) {
                _this.pullSession2Fail(groupId, response)
              }
            },
            fail: function (response) {
              _this.$root.imdemo.utils.onError({
                code: 'onPullSessionFailedCallback',
                message: '拉取会话（指定）失败！',
                data: response
              })
              _this.pullSession2Fail(groupId, response)
          ...
          if (!_this.cache.msgAll[groupId].pullMsg) {
            ...
            _this.$root.imdemo.conn.pullMsg({
              ...
              success: function (response) {
                console.log(response)
                _this.pullMsgSuccess(groupId, response)
              },
              fail: function (response) {
                _this.$root.imdemo.utils.onError({
                  code: 'onPullMsgFailedCallback',
                  message: '拉取消息（未读消息）失败！',
                  data: response
                })
      ...
      loginSuccess: function (response) {
        ...
        _this.$root.imdemo.conn.pullSession({
          recentDays: 0,
          success: function (response) {
            _this.pullSessionSuccess(response)
          },
          fail: function (response) {
            _this.$root.imdemo.utils.onError({
              code: 'onPullSessionFailedCallback',
      ...
      pullSessionSuccess: function (response) {
        var _this = this
        var timeoutMsgArray = []
      ...
      pullSession2Success: function (groupId, response) {
        var _this = this
        var groupType = _this.cache.msgAll[groupId].groupType
        var basicInfo = _this.cache.msgAll[groupId].basicInfo
        var timeoutMsgArray = []
        for (var i = 0; i < response.msgBody.SessionList.length; i++) {
          var item = response.msgBody.SessionList[i]
          var filter = false
      ...
      pullSession2Fail: function (groupId, response) {
        var _this = this
        var groupType = _this.cache.msgAll[groupId].groupType
        var basicInfo = _this.cache.msgAll[groupId].basicInfo
        var timeoutMsgArray = []
      ...
      pullMsgSuccess: function (groupId, response) {
        var _this = this
        _this.cache.msgAll[groupId].pullMsg = true
